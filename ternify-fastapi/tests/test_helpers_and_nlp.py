...
# ====================================================================
# NLP token_sort_ratio fallback
# ====================================================================
class TestNlpTokenSortRatioFallback:
    """
    Tujuan: Memastikan fallback token_sort_ratio menangani kasus di mana
    OCR membaca kata dengan urutan terbalik (mis. 'makan nafsu' → 'nafsu makan').
    """

    def test_reordered_words_corrected(self):
        """Kata dengan urutan terbalik dikoreksi via token_sort_ratio."""
        # 'rendah nafsu makan' → harus match 'nafsu makan rendah' di KB_STATUS
        result = nlp_text_correction("rendah nafsu makan", KB_STATUS, threshold=70)
        assert result == "nafsu makan rendah"

    def test_exact_match_not_changed(self):
        """Exact match (fuzz.ratio=100) tetap tidak diubah."""
        result = nlp_text_correction("nafsu makan rendah", KB_STATUS, threshold=70)
        assert result == "nafsu makan rendah"

    def test_primary_ratio_still_works(self):
        """Primary fuzz.ratio masih bekerja untuk typo biasa."""
        result = nlp_text_correction("Merimo", KB_RAS)
        assert result == "Merino"

    def test_short_word_skipped(self):
        """Kata pendek (≤2 char) tetap di-skip oleh kedua strategi."""
        result = nlp_text_correction("ab", KB_RAS, threshold=70)
        assert result == "ab"

    def test_no_match_either_scorer(self):
        """Kata yang jauh dari semua KB → tidak dikoreksi."""
        result = nlp_text_correction("xyzqwerty", KB_GEJALA, threshold=70)
...
# _extract_lines_from_tess_data
# ====================================================================
class TestExtractLinesFromTessData:
    """
    Tujuan: Memastikan _extract_lines_from_tess_data mengelompokkan
    kata-kata dari Tesseract output berdasarkan Y-coordinate dan
    mengurutkan per baris berdasarkan X-coordinate.
    """

    def test_single_line(self):
        """Satu baris teks dengan beberapa kata."""
        data = {
            "text": ["Hello", "World"],
            "conf": [90, 85],
            "top": [10, 10],
        ...
        result = _extract_lines_from_tess_data(data)
        assert result == "Hello World"

    def test_multiple_lines(self):
        """Beberapa baris teks terpisah berdasarkan Y-coordinate."""
        data = {
            "text": ["Line1", "Line2"],
            "conf": [90, 85],
            "top": [10, 100],
    ...
    def test_nearby_y_grouped(self):
        """Kata dengan Y-coordinate dekat (< 20px) digabung jadi satu baris."""
        data = {
            "text": ["Word1", "Word2"],
            "conf": [90, 85],
            "top": [10, 15],  # selisih 5px < 20px → satu baris
            "left": [50, 100],
        }
        result = _extract_lines_from_tess_data(data)
    ...
    def test_x_ordering(self):
        """Kata diurutkan berdasarkan X-coordinate dalam satu baris."""
        data = {
            "text": ["Second", "First"],
            "conf": [90, 85],
            "top": [10, 10],
            "left": [200, 50],  # 'First' di kiri, 'Second' di kanan
        }
        result = _extract_lines_from_tess_data(data)
        assert result == "First Second"

    def test_empty_data(self):
        """Data kosong → string kosong."""
